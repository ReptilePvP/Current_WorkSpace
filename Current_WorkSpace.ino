...
void drawMainDisplay(float temperature) {
    ...
    if (isValidTemperature(temperature)) {
        ...
        if (state.isMonitoring) {
            state.updateStatusWithPriority(statusMsg, tempColor, PRIORITY_TEMP);
...
void drawStatusBox() {
    ...
    if (lastStatus != state.statusMessage || lastColor != state.statusColor || forceRedraw) {
...
void onCloudRestartChange() {
    if (cloudRestart) {
        ...
        for (int i = 3; i > 0; i--) {
            ...
            state.updateStatus(countMsg, Config::Display::COLOR_WARNING);
        ...
        CoreS3.Display.fillScreen(Config::Display::COLOR_BACKGROUND);
        state.updateStatus("Restarting now!", Config::Display::COLOR_WARNING);
...
void handleCloudConnection() {
    ...
    if (currentlyConnected != state.cloudConnected) {
        ...
        if (currentlyConnected) {
            ...
            state.updateStatusWithPriority("Cloud Connected", Config::Display::COLOR_SUCCESS, PRIORITY_CLOUD);
            if (settings.soundEnabled) {
                playSuccessSound();
            }
        } else {
            // Lost connection
            state.updateStatusWithPriority("Cloud Disconnected", Config::Display::COLOR_ERROR, PRIORITY_CLOUD);
            if (settings.soundEnabled) {
                playErrorSound();
            }
        }
    ...
    if (!currentlyConnected) {
        ...
        if (currentTime - state.lastReconnectAttempt >= interval) {
            ...
            state.updateStatusWithPriority(statusMsg, Config::Display::COLOR_WARNING, PRIORITY_CLOUD);
            ...
            if (state.reconnectAttempts >= state.MAX_RECONNECT_ATTEMPTS) {
                state.updateStatusWithPriority("Waiting 1 minute...", Config::Display::COLOR_WARNING, PRIORITY_CLOUD);
            }
        }
    }
}
